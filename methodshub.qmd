...
`webtrackR` accepts raw webtracking data as provided by GESIS and includes a sample dataset of raw webtracking data. At minimum, the raw data must contain a participant identifier (`panelist_id`), the visited `url`, and a `timestamp`. The bundled example dataset `testdt_tracking` illustrates this format:
...
knitr::kable(head(testdt_tracking, 5))
```
...
The functions in webtrackR return processed `wt_dt` objects (enhanced data frames) that contain enriched, cleaned, or summarized web tracking data. These objects remain in-memory within R and can be directly analyzed, visualized, or explicitly exported by the user into external file formats such as CSV or RDS. An abbreviated example of a summarized output object is shown at the end of the [Example Code](#example-code) section below.
...
# inspect the first rows of the wt_dt object
knitr::kable(head(my_wt_data, 5))
```
...
```{r}
#| eval: false
# save the processed data explicitly if needed
write.csv(my_wt_data, "processed_webtracking.csv", row.names = FALSE)
```
...
# show an abbreviated example of the aggregated output
knitr::kable(head(wt_summ, 5))
```
